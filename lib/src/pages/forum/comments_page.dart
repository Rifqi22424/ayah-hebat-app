...
class _CommentsPageState extends State<CommentsPage> {
          ...
          if (type == "report") {
            ...
            return AlertDialog(
              shape: RoundedRectangleBorder(
                  borderRadius: BorderRadius.circular(24)),
              title: Text("Peringatan", style: AppStyles.mediumTextStyle),
              content: Text("Apakah anda ingin menghapus comment tersebut?",
                  style: AppStyles.heading3TextStyle),
              actions: [
                TextButton(
                  style: TextButton.styleFrom(
            ...
            Expanded(
              child: RefreshIndicator(
                ...
                child: ListView(
                  ...
                  children: [
                    content(linkImage, post, index, context),
                  ],
                ),
              ),
            ),
            textFieldSection(context, post),
          ],
        ),
      ),
    );
  ...
  Container textFieldSection(BuildContext context, Post post) {
    return Container(
                ...
                child: Container(
                    ...
                    child: Consumer<CommentProvider>(
                      builder: (context, commentProv, child) {
                        return commentProv.iconState == IconState.loading
                            ? const Center(
                                child: SizedBox(
                                  height: 20,
                                  width: 20,
  ...
  Column content(String linkImage, Post post, int index, BuildContext context) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Padding(
          padding: const EdgeInsets.all(16.0),
          child: postSection(linkImage, post, index),
        ),
        const Divider(color: AppColors.grey, thickness: 2, height: 6),
        filterFeature(context, post),
        commentSection(post),
      ],
    );
  }

  Consumer<CommentProvider> commentSection(Post post) {
    return Consumer<CommentProvider>(
      builder: (context, commentProv, child) {
        print("offset: " + offset.toString());
        if (commentProv.commentState == CommentState.loading &&
        ...
        } else if (commentProv.commentState == CommentState.error) {
          return Center(
            child: Text(
                'Error: ${commentProv.errorCommentMessage ?? 'An error occurred'}'),
          );
        } else if (commentProv.comments.isEmpty) {
          return Center(
            child: Text('Belum ada komentar', style: AppStyles.labelTextStyle),
          );
        } else {
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              if (index == commentProv.comments.length) {
                return commentProv.commentState == CommentState.loading
                    ? const SizedBox(
                        height: 50,
                        width: 50,
                        child: Center(
              ...
              return Column(
                children: [
                  InkWell(
                    ...
                    onLongPress: () {
                      FocusScope.of(context).unfocus();
                      showMoreDialog(context, comment.isMine, comment.id, true);
                    },
                    child: Padding(
                          ...
                          children: [
                            ...
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  ExpandableText(
                                      body: comment.body, isPost: false),
                            ...
                            Align(
                              ...
                              child: IconButton(
                                  onPressed: () {
                                    print("status like: " +
                                        comment.isLikedByMe.toString());
                                    EditReplyState editReplyState =
                                        Provider.of<CommentProvider>(context,
                                                listen: false)
                                            .editReplyState;
                                    if (editReplyState ==
                                        EditReplyState.loading) {
                                      print("cek");
                                    } else {
                                      Provider.of<CommentProvider>(context,
                                              listen: false)
                  ...
                  replySection(comment, commentProv)
                ],
              );
            },
          );
  ...
  ListView replySection(Comment comment, CommentProvider commentProv) {
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        if (index == comment.replies.length) {
          return commentProv.replyState == ReplyState.loading
              ? const Center(
                  child: SizedBox(
                    height: 20,
                    width: 20,
        ...
        return InkWell(
          ...
          onLongPress: () {
            FocusScope.of(context).unfocus();
            showMoreDialog(context, reply.isMine, reply.id, false);
          },
          child: Container(
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 4),
                        ExpandableText(body: reply.body, isPost: false),
                        const SizedBox(height: 2),
                        Row(
                          ...
                          children: [
                            Row(
                              children: [
                                Text(reply.replyLikesCount.toString(),
                                    style: AppStyles.miniHintTextStyle),
                                const SizedBox(width: 2),
                                Text('likes',
                                    style: AppStyles.miniHintTextStyle)
                            ...
                            InkWell(
                                onTap: () {
                                  ...
                                  replyToReplyId = index;
                                  replyToReplyName = reply.user.profile.nama;
                                  isReply = true;
  ...
  SizedBox filterFeature(BuildContext context, Post post) {
    return SizedBox(
      height: 40,
      child: DropdownButton(
        ...
        items: <String>['Terbaru', 'Terpopuler'].map((String value) {
  ...
  Column postSection(String linkImage, Post post, int index) {
    return Column(
      children: [
        Row(
          children: [
            ...
            return Row(
              children: [
                SizedBox(
                  child: Row(
                    children: [
                      Text(postData.count.comments.toString()),
                      SizedBox(width: 2),
                      IconButton(
                ...
                SizedBox(
                  child: Row(
                    children: [
                      Text(postData.count.postDislikes.toString()),
                      SizedBox(width: 2),
                      IconButton(
                          padding: EdgeInsets.zero,
                          constraints: BoxConstraints(),
                ...
                SizedBox(
                  child: Row(
                    children: [
                      Text(postData.count.postLikes.toString()),
                      SizedBox(width: 2),
                      IconButton(
                        padding: EdgeInsets.zero,
                        constraints: BoxConstraints(),
